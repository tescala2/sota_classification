...
def convnext_tiny(pretrained: bool = False, **kwargs: Any) -> ConvNeXt:
    ...
    if pretrained:
        pretrained_state_dict = model_zoo.load_url(model_urls['convnext_tiny'])
...
def convnext_small(pretrained: bool = False, **kwargs: Any) -> ConvNeXt:
    ...
    model = ConvNeXt(block_setting, stochastic_depth_prob=stochastic_depth_prob, **kwargs)
    if pretrained:
        pretrained_state_dict = model_zoo.load_url(model_urls['convnext_small'])
        now_state_dict = model.state_dict()
        now_state_dict.update(pretrained_state_dict)
        model.load_state_dict(now_state_dict)
    return model
...
def convnext_base(pretrained: bool = False, **kwargs: Any) -> ConvNeXt:
    ...
    model = ConvNeXt(block_setting, stochastic_depth_prob=stochastic_depth_prob, **kwargs)
    if pretrained:
        pretrained_state_dict = model_zoo.load_url(model_urls['convnext_base'])
        now_state_dict = model.state_dict()
        now_state_dict.update(pretrained_state_dict)
        model.load_state_dict(now_state_dict)
    return model
...
def convnext_large(pretrained: bool = False, **kwargs: Any) -> ConvNeXt:
    ...
    model = ConvNeXt(block_setting, stochastic_depth_prob=stochastic_depth_prob, **kwargs)
    if pretrained:
        pretrained_state_dict = model_zoo.load_url(model_urls['convnext_large'])
        now_state_dict = model.state_dict()
        now_state_dict.update(pretrained_state_dict)
        model.load_state_dict(now_state_dict)
    return model
