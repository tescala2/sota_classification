...
from models.utils import ReparamLargeKernelConv, MobileOneBlock
...
class FastViT(nn.Module):
    ...
    def __init__(
        ...
        # Convolutional stem
        self.stem = convolutional_stem(3, embed_dims[0], self.inference_mode)

        # Build the main stages of the network architecture
        self.layer1 = self._make_layer(0, pos_embs, embed_dims, token_mixers, mlp_ratios, downsamples)
        self.layer2 = self._make_layer(1, pos_embs, embed_dims, token_mixers, mlp_ratios, downsamples)
        self.layer3 = self._make_layer(2, pos_embs, embed_dims, token_mixers, mlp_ratios, downsamples)
        self.layer4 = self._make_layer(3, pos_embs, embed_dims, token_mixers, mlp_ratios, downsamples)
    ...
    def _make_layer(self, i, pos_embs, embed_dims, token_mixers, mlp_ratios, downsamples):
        layer = []
        if pos_embs[i] is not None:
            layer.append(
                pos_embs[i](
                    embed_dims[i], embed_dims[i], inference_mode=self.inference_mode
                )
            )
        stage = basic_blocks(
            embed_dims[i],
...
if __name__ == "__main__":
    ...
    model = fastvit_ma36(pretrained=False, num_classes=num_classes).to(device)
