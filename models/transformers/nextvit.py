...
__all__ = ['NextViT', 'nextvit_small', 'nextvit_base', 'nextvit_large']

model_urls = {
    'nextvit_small': 'https://drive.google.com/file/d/1b7ChnlT3uG3pTaZjtwYtnAaxAESF0MqK',
    'nextvit_base': 'https://drive.google.com/file/d/1qYtex2EZ23TzVqaFlGh14hAxVjfuILWv',
    'nextvit_large': 'https://drive.google.com/file/d/1F5iLING3RzNbuZb4RwL7NLZZmJulKW3N'
}
...
def nextvit_small(pretrained: bool = False, **kwargs) -> NextViT:
    model = NextViT(stem_chs=[64, 32, 64], depths=[3, 4, 10, 3], path_dropout=0.1, **kwargs)
    if pretrained:
        pretrained_state_dict = model_zoo.load_url(model_urls['nextvit_small'])
...
def nextvit_base(pretrained: bool = False, **kwargs) -> NextViT:
    model = NextViT(stem_chs=[64, 32, 64], depths=[3, 4, 20, 3], path_dropout=0.2, **kwargs)
    if pretrained:
        pretrained_state_dict = model_zoo.load_url(model_urls['nextvit_base'])
        now_state_dict = model.state_dict()
        now_state_dict.update(pretrained_state_dict)
        model.load_state_dict(now_state_dict, strict=False)
    return model
...
def nextvit_large(pretrained: bool = False, **kwargs) -> NextViT:
    model = NextViT(stem_chs=[64, 32, 64], depths=[3, 4, 30, 3], path_dropout=0.2, **kwargs)
    if pretrained:
        pretrained_state_dict = model_zoo.load_url(model_urls['nextvit_large'])
        now_state_dict = model.state_dict()
        now_state_dict.update(pretrained_state_dict)
        model.load_state_dict(now_state_dict, strict=False)
    return model
...
if __name__ == '__main__':
    ...
    model = nextvit_large(pretrained=False, num_classes=num_classes).to(device)
