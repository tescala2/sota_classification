...
__all__ = ["SwinTransformer", "swin_t", "swin_s", "swin_b", "swin_v2_t", "swin_v2_s", "swin_v2_b"]
...
def swin_t(pretrained: bool = False, **kwargs: Any) -> SwinTransformer:
    ...
    if pretrained:
        pretrained_state_dict = model_zoo.load_url(model_urls['swin_t'])
...
def swin_s(pretrained: bool = False, **kwargs: Any) -> SwinTransformer:
    ...
    if pretrained:
        pretrained_state_dict = model_zoo.load_url(model_urls['swin_s'])
        now_state_dict = model.state_dict()
        now_state_dict.update(pretrained_state_dict)
        model.load_state_dict(now_state_dict, strict=False)
    return model
...
def swin_b(pretrained: bool = False, **kwargs: Any) -> SwinTransformer:
    ...
    if pretrained:
        pretrained_state_dict = model_zoo.load_url(model_urls['swin_b'])
        now_state_dict = model.state_dict()
        now_state_dict.update(pretrained_state_dict)
        model.load_state_dict(now_state_dict, strict=False)
    return model
...
def swin_v2_t(pretrained: bool = False, **kwargs: Any) -> SwinTransformer:
    ...
    if pretrained:
        pretrained_state_dict = model_zoo.load_url(model_urls['swin_v2_t'])
        now_state_dict = model.state_dict()
        now_state_dict.update(pretrained_state_dict)
        model.load_state_dict(now_state_dict, strict=False)
    return model
...
def swin_v2_s(pretrained: bool = False, **kwargs: Any) -> SwinTransformer:
    ...
    if pretrained:
        pretrained_state_dict = model_zoo.load_url(model_urls['swin_v2_s'])
        now_state_dict = model.state_dict()
        now_state_dict.update(pretrained_state_dict)
        model.load_state_dict(now_state_dict, strict=False)
    return model
...
def swin_v2_b(pretrained: bool = False, **kwargs: Any) -> SwinTransformer:
    ...
    if pretrained:
        pretrained_state_dict = model_zoo.load_url(model_urls['swin_v2_b'])
        now_state_dict = model.state_dict()
        now_state_dict.update(pretrained_state_dict)
        model.load_state_dict(now_state_dict, strict=False)
    return model
...
if __name__ == '__main__':
    ...
    # NotImplementedError: The operator 'aten::roll' is not currently implemented for the MPS device. If you want this
    # op to be added in priority during the prototype phase of this feature, please comment on
